...
Т.к. перед нам стоит задача:
- декомпозиции предметной области, т.е. "распила" монолита на микросервисы;
- при этом необходимо сохранить возможность клиентам, которые не сразу обновляют приложения, ходить через старый API;
- а также необходима гибкость в выборе тех типов микросервисного взаимодействия, которые покажутся целесообразными.

При решении таких задача возникает соблазн создать такую инфраструктуру, которая решит сразу все эти проблемы.
Предлагается внедрение микросервиса revers-proxy gateway или иначе называемом шлюзом или единой точкой входа в систему. 
...
- собрать cluster Clickhouse, который будет состоять из одной cluster node'ы и двух node для шардирования;
