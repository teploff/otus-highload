...
//Log if first key = msg than first value will be interpreted as zap logger message (this is important for sentry
//integration).
...
func NewZapSugarLogger(logger *zap.Logger, level zapcore.Level) log.Logger {
	sugarLogger := logger.WithOptions(zap.AddCallerSkip(countSkips)).Sugar()
