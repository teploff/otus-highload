...
### Результаты
Оценив результаты выше, можно прийти к удручающему выводу: tarantool на каких-то точечных, запроса работает быстро,
однако, если дать на него нагрузку, он начинает вести себя попросту неадекватно и не удовлетворяет элементарным
требованиям работоспособности системы. Однако в свою очередь MySQL показывает надежный результат
