...
func (s *socialService) GetQuestionnaires(ctx context.Context, userID string, offset, limit int) ([]*domain.Questionnaire, int, error) {
	...
	users, err := s.repository.GetByLimitAndOffsetExceptUserID(tx, userID, offset, limit)
	if err != nil {
		return nil, 0, err
	}
...
func (s *socialService) GetQuestionnairesByNameAndSurname(ctx context.Context, prefix string, offset, limit int) (
	[]*domain.Questionnaire, int, error) {
	tx, err := s.repository.GetTx(ctx)
	if err != nil {
		return nil, 0, err
	}

	users, count, err := s.repository.GetByPrefixOfNameAndSurname(tx, prefix, offset, limit)
	if err != nil {
		return nil, 0, err
	}
	...
	return questionnaires, count, s.repository.CommitTx(tx)
}
