...
<a name="work-execute-lady-gaga"></a>
#### "Эффект *Lady Gaga*" с последующим переводом на другую node'у
...
<a name="work-execute-resharding"></a>
#### Процедура решардинга
...
- была реализована масштабируемая подсистема диалогов;
- был собран кластер из шести узлов ClickHose (одна - кластер node'а, а остальные - shard'ы)
- был реализован алгоритм выбора ключа шардирования с учетом "эффекта Леди Гаги";
- был описан и реализован процесс решардинга без даунтайма.